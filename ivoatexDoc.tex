...
browser UI.  Make sure this repository uses the template TODO
\item Locally clone the freshly created repository
\item \verb|git checkout -b initial-import|
\item \verb|git pull --allow-unrelated-histories <original-repo>|
\item \verb|git push|
\item Create a PR using github's browser UI
\item Give the WG chair and the editor the following privileges: TODO
\item Notify the WG of the move via the WG's mailing list
\end{enumerate}
