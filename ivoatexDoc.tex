...
\texttt{archdiag.svg} from \texttt{FIGURES}.

To prepare an architecture diagram, copy
\texttt{ivoatex/archdiag-full.xml} to \texttt{archdiag.xml}.  Then edit
\texttt{archdiag.xml} and remove all references to standards unrelated
to the current document.  As a rule, all \texttt{rec} elements for
standards not mentioned in
``Role within the IVOA Architecture'' should be removed.  The
architecture diagram will be built by saying \texttt{make archdiag.pdf}.
