...
IVOA repositories follow most open source projects in using a fork and pull request workflow for contributing updates. 
...
When setting up your environment for a first time commit, we recommend following this `triangle' workflow to make later contributions easier. 
First fork the official repository through the GitHub web page (add image)

Then on your local computer, clone the official repository and then add your fork as a remote repository. Note here that we use a read-only clone (git://) from the IVOA repository to avoid the potential for future mistakes. 
\begin{lstlisting}[language=sh]
...
Now to prepare for your change, make sure you are up to date with the official repository and create a branch for your work.
\begin{lstlisting}[language=sh]
git checkout main
...
Once you are happy with the change (and we do recommend building the PDF document locally to review it) you can save it to git and send it to your forked repository.
...
\end{lstlisting}
 ...
 and then on the GitHub web interface create your Pull Request to contribute it back to the official repository.
