...
amount of spurious error messages during HTML generation.
...
As of version 0.5 of \ivoatex, only \texword{footnotesize} is actually
...
documentation from XML Schema files into the standard document (used,
e.g., in TAPRegExt 1.1) or obtaining column metadata for standard
data models from a live TAP\_SCHEMA (used, e.g., in RegTAP 1.0).
...
We mention in passing that in addition to allowing arbitrary shell
commands, \texttt{update\_generated.py} has a facility that allows
calling special python functions from within documents: Commands starting
with an exclamation mark (``!'') are interpreted as such calls to
functions defined within \texttt{update\_generated.py}.
Currently, the only such builtin command extracts documentation from a
...
Data Collections Within Services} IVOA Note.}.  For now, this should be
considered an unstable experiment, but authors wanting to try inline
python for content generation are welcome to do so.
...
\subsubsection{XML Schema documentation}

A common use case for this facility is building documentation from XML
schema files.  To support it, \ivoatex\ comes with an XSLT file
\texttt{schemadoc.xslt} that generates \TeX~source for one type, the
name of which is passed in through a stylesheet parameter named
\texttt{destType}.  Since \ivoatex\ itself depends on xsltproc, the
recommended way to use the facility looks like this:

\lstinputlisting[language=tex,basicstyle=\footnotesize]{schemainclusion.tex}

Note that currently the entire command line must be in one physical line
in the file, so the recommendation to keep source lines shorter than 72
characters clearly does not apply here.
...
legitimate cases when something needs extra styling in HTML that simply
somes out right in the PDF output.  In such cases, a standard CSS file can
...
\begin{lstlisting}
\comicstuff{If this is comic sans, your webbrowser is permissive.}
\end{lstlisting}
